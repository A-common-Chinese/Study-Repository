...
def ensure_weights_exist(file_name="weights.json"):
    ...
    if not os.path.exists(file_path):
        sample_data = {
            "发挥系数": {
                "高手": {"穿深": 1, "装填": 2, "辅助设备": 1.2, "双机响应速度": 2,
                ...
                "菜鸟": {"穿深": 1, "装填": 0.5, "辅助设备": 0.5, "双机响应速度": 0.5,
                        "弱点分布": 0.7, "防护面积": 1, "生存性": 1, "倒车速度": 0.7,
                        "前进能力": 0.4, "小范围综合机动性": 0.5}
            },
            "依赖系数": {
                "高手": {"穿深": 0.5, "装填": 1.5, "辅助设备": 0.5, "双机响应速度": 0.9,
                        "弱点分布": 0.5, "防护面积": 0.5, "生存性": 1, "倒车速度": 1,
                        "前进能力": 2, "小范围综合机动性": 2},
                "一般玩家": {"穿深": 1, "装填": 1, "辅助设备": 1, "双机响应速度": 1,
                          "弱点分布": 1, "防护面积": 1, "生存性": 1, "倒车速度": 1,
                          "前进能力": 1, "小范围综合机动性": 1},
                "菜鸟": {"穿深": 2, "装填": 2, "辅助设备": 1.5, "双机响应速度": 2,
                        "弱点分布": 2, "防护面积": 2, "生存性": 2, "倒车速度": 2,
                        "前进能力": 0.7, "小范围综合机动性": 1.5}
            }
        }
...
if __name__ == "__main__":
    ...
    # 映射玩家层次 -> 对应的双系数字典
    level_coeffs = {
        "高手": high_level_weight,
        "一般玩家": middle_level_weight,
        "菜鸟": low_level_weight,
        "综合": None   # 综合层特殊处理
    }
    ...
    for tank, scores in vehicle_scores.items():
        ...
        for level in ["高手", "一般玩家", "菜鸟"]:
            coeffs = level_coeffs[level]
            for param, obj_score in scores.items():
                if param in coeffs and param not in ["俯角", "APS", "LWS"]:
                    f = coeffs[param]["发挥系数"]
                    y = coeffs[param]["依赖系数"]
                    speciality = abs(f - 1) + abs(y - 1)
                    value = obj_score * (1 + k * speciality)
                    radar_data[tank][level][param] = round(value, 2)

        # 综合层：取三个层次的平均值
        for param in scores:
            if param in ["俯角", "APS", "LWS"]:
                continue
            vals = []
            for level in ["高手", "一般玩家", "菜鸟"]:
                if param in radar_data[tank][level]:
                    vals.append(radar_data[tank][level][param])
            if vals:
                radar_data[tank]["综合"][param] = round(sum(vals) / len(vals), 2)
